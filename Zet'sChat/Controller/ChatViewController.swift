...
class ChatViewController: UIViewController {
    ...
    func loadMessages() {
        ...
        db.collection(K.FStore.collectionName)
            ...
            if let e = error {
                self.goToView(with: "There was an issue retrieving data from Firestore. \(e.localizedDescription)")
            } else {
    ...
    @IBAction func sendPressed(_ sender: UIButton) {
        ...
        if let messageBody = messageTextfield.text, let messageSender = Auth.auth().currentUser?.email {
            db.collection(K.FStore.collectionName).addDocument(data: [
                ...
                if let e = error {
                    self.goToView(with: "There was an issue saving data to firestore, \(e.localizedDescription)")
                } else {
                    DispatchQueue.main.async {
    ...
    @IBAction func logOutPressed(_ sender: UIBarButtonItem) {
        ...
        do {
            ...
            self.goToView(with: "Error signing out: \(signOutError.localizedDescription)")
        }
    }
...
extension ChatViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if message.sender == Auth.auth().currentUser?.email {
            ...
            cell.label.textColor = UIColor(named: K.BrandColors.brandPurple)
        }
